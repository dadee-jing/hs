...
public class WeightDataController extends BaseController {
    ...
    public AjaxResult export(com.ruoyi.duge.domain.WeightData data) throws FileNotFoundException {
        ...
        for (int i = 0; i < weightDataList.size(); i++) {
            ...
            list.add(String.format("%.2f", weightData.getOverWeight().doubleValue() / weightData.getLimitWeight().doubleValue() * 100) + "%");
    ...
    public String detail(@PathVariable("id") Long id, ModelMap mmap, HttpServletRequest request) {
        ...
        mmap.put("data", data);
        mmap.put("overRate", String.format("%.2f", data.getOverWeight().doubleValue() / data.getLimitWeight().doubleValue() * 100) + "%");
        //mmap.put("ctxPath", request.getContextPath() + "\\");
        mmap.put("ctxPath", configDataService.getConfigValue("nginx_url")
                + data.getStationId() + "/" + DateFormatUtils.format(data.getWeightingDate(), "yyyyMMdd") + "/");
    ...
    public WeightData refreshLast() {
        com.ruoyi.duge.domain.WeightData data = dataService.selectLast();
        data.getParams().put("overRate", String.format("%.2f", data.getOverWeight().doubleValue() / data.getLimitWeight().doubleValue() * 100) + "%");
        data.getParams().put("weightDate", DateFormatUtils.format(data.getWeightingDate(), "yyyy-MM-dd HH:mm:ss"));
        data.getParams().put("ctxPath", configDataService.getConfigValue("nginx_url")
                + data.getStationId() + "/" + DateFormatUtils.format(data.getWeightingDate(), "yyyyMMdd") + "/");
        return data;
    }
